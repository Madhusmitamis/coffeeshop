...
public interface TuoteRepository extends JpaRepository<Tuote, Long> {
        ...
        // @Query("SELECT t FROM Tuote t JOIN FETCH t.osasto o WHERE o.id = :osastoID OR
        // o.osastoIdp = :osastoID")
        // Page<Tuote> findProductsByOsastoID(@Param("osastoID") Long osastoID, Pageable
        // pageable);

        // @Query("SELECT t FROM Tuote t JOIN FETCH t.osasto o WHERE o.id IN :osastoIDs
        ...
        @Query("SELECT DISTINCT t FROM Tuote t JOIN t.osasto o WHERE o.id = :osastoID OR o.osastoIdp = :osastoID")
        ...
        @Query("SELECT DISTINCT t FROM Tuote t JOIN t.osasto o WHERE o.id IN :osastoIDs OR o.osastoIdp IN :osastoIDs")
